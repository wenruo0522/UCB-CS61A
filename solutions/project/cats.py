...
def about(subject):
    ...
    def is_in(test_paragraphs):
        is_check = False
        for phrase in subject:
            test_paragraphs_list = split(test_paragraphs)
            for test_paragraph in test_paragraphs_list:
                if phrase == remove_punctuation(lower(test_paragraph)):
                    is_check = True
        return is_check
    ...
    if len(typed_words) == 0 and len(source_words) == 0:
        ...
        for i in range(min_len):
            if typed_words[i] == source_words[i]:
                ...
                incorrect_count += 1
        if len(typed_words) > min_len:
            incorrect_count += len(typed_words) - min_len
        return correct_count * 100 / (incorrect_count + correct_count)
    # END PROBLEM 3
...
def report_progress(typed, source, user_id, upload):
    ...
    d = {"id": user_id, "progress": correct_ratio}
    upload(d)
    print(correct_ratio)
    # END PROBLEM 8
...
def time_per_word(words, timestamps_per_player):
    ...
    "*** YOUR CODE HERE ***"
    timestamps_result = []
    for index in range(len(timestamps_per_player)):
        timestamps_player = timestamps_per_player[index]
        sub_lists = []
        for subIndex in range(len(timestamps_player) - 1):
            sub_lists += [timestamps_player[subIndex + 1] - timestamps_player[subIndex]]
        timestamps_result += [sub_lists]
    return match(words, timestamps_result)
    # END PROBLEM 9
...
def fastest_words(match):
    ...
    for word_index in word_indices:
        ...
        for play_index in player_indices:
            player_time = time(match, play_index, word_index)
            if first_cycle:
                min_time = player_time
                actual_index = play_index
                first_cycle = False
            else:
                if player_time < min_time:
                    min_time = player_time
                    actual_index = play_index
        result[actual_index].append(get_word(match, word_index))
    return result
    # END PROBLEM 10
