...
def boar_brawl(player_score, opponent_score):
    ...
    # don"t assume opponent_score is below 100
    if tens_digit > 10:
...
def sus_points(score):
    ...
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    factor_number = num_factors(score)
    if factor_number == 3 or factor_number == 4:
        is_prime_number = False
        while not is_prime_number:
            score += 1
            is_prime_number = is_prime(score)
    return score
    # END PROBLEM 4
...
def is_always_roll(strategy, goal=GOAL):
    ...
    for score0 in range(goal):
        for score1 in range(goal):
            new_roll = strategy(score0, score1)
            if new_roll != roll:
                change_roll = False
    return change_roll
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, samples_count=1000):
    ...
    while start <= end:
        average_score = average_dice(start, dice)
